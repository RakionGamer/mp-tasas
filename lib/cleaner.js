...
export async function validarTasasPorCantidad(jsonData) {
  ...
  const esEnvioVenezuela = /Env[ií]os? desde Venezuela( a)?/i.test(textoCompleto);
  const esEnvioBrasil = /desde\s+brasil/i.test(textoCompleto);
  const esEnvioPeru = /desde\s+per[uú]/i.test(textoCompleto);
  const esEnvioColombia = /desde\s+colombia/i.test(textoCompleto);
  ...
  let configAUsar;
  if (esEnvioChile) configAUsar = configuraciones[3];
  ...
  else if (esCambios) configAUsar = configuraciones[2];
