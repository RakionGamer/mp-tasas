...
export async function validarTasasPorCantidad(jsonData) {
  ...
  const esEnvioBrasil = /Env[ií]os? desde Brasil/i.test(textoCompleto);
  const esEnvioPeru = /Env[ií]os? desde Per[uú]/i.test(textoCompleto);
  const esEnvioColombia = /Env[ií]os? desde Colombia/i.test(textoCompleto);
