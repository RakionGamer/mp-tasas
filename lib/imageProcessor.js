...
export async function createImageWithRates(extractedData) {
    try {
        ...
        // Función helper ultra simple que siempre funciona
        const drawTextWithStroke = (text, x, y, fontSize = 48, strokeWidth = 4) => {
            if (!text || text === 'undefined' || text === 'null') {
                console.warn(`Texto inválido para posición (${x}, ${y}):`, text);
                return;
            }

            // Solo usar monospace - garantizado en cualquier sistema
            ctx.font = `${fontSize}px monospace`;
            ctx.fontWeight = 'normal'; // No usar bold que puede fallar

            const cleanText = String(text).trim();
            console.log(`Dibujando: "${cleanText}" en (${x}, ${y})`);

            ctx.lineWidth = strokeWidth;
            ctx.strokeText(cleanText, x, y);
            ctx.fillText(cleanText, x, y);

            console.log('Canvas fonts:', ctx.font);
            console.log('Platform:', process.platform);

            // Test simple de texto
