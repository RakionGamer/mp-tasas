...
// TODO: Reemplazar con el enlace correcto de Cloudinary de la plantilla base de PERU
const baseImageUrl =
  "https://res.cloudinary.com/dvh3nrsun/image/upload/v1781115458/6_nufloh.png";

export async function createImageWithRatesPeru(dataPeru, extractedData) {
  try {
    ...
    const tasasMapping = {
      colombia: numeros?.tasaPeruColombia || dataPeru?.COLOMBIA || "No disponible",
      argentina: numeros?.tasaPeruArgentina || dataPeru?.ARGENTINA || "No disponible",
      brasil: numeros?.tasaPeruBrasil || dataPeru?.BRASIL || "No disponible",
      chile: numeros?.tasaPeruChile || dataPeru?.CHILE || "No disponible",
      panama: numeros?.tasaPeruPanama || dataPeru?.PANAMA || "No disponible",
      dominicana: numeros?.tasaPeruDominicana || dataPeru?.DOMINICANA || "No disponible",
      usa: numeros?.tasaPeruUSA || "No disponible",
      españa: numeros?.tasaPeruEspaña || "No disponible",
      ecuador: numeros?.tasaPeruEcuador || dataPeru?.ECUADOR || "No disponible",
      mexico: numeros?.tasaPeruMexico || dataPeru?.MEXICO || "No disponible",
      bolivia: numeros?.tasaPeruBolivia || dataPeru?.BOLIVIA || "No disponible",
    };
      ...
      const stream = cloudinary.v2.uploader.upload_stream(
        {
          ...
          resource_type: "image",
          format: "jpg",
          public_id: `rates_peru_${Date.now()}`,
        },
        (error, result) => {
    ...
    console.error("Error procesando imagen Peru:", error);
    throw error;
  }
}
